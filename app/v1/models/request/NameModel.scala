...
object NameModel extends Logging {
  ...
  private val titleValidationError: JsonValidationError = JsonValidationError(
    "Title failed validation. Must be one of: \'NOT KNOWN, MR, " +
      "MRS, MISS, MS, DR, REV\'"
  )

  private def nameValidationError: String => JsonValidationError = fieldName =>
    JsonValidationError(
      s"Unable to validate $fieldName. " +
        s"Ensure it matches the regex."
    )

  private def typeValidationError: JsonValidationError = JsonValidationError(
    "Name Type failed validation. Must be one of: \'REGISTERED, ALIAS\'"
  )
  ...
  private[models] def validateDateAsPriorDate(
    earlierDate: Option[DateModel],
    laterDate: Option[DateModel],
  ...
  implicit val reads: Reads[NameModel] = for {
    ...
    secondForename <- secondForenamePath
                        .readNullable[String]
                        .filter(nameValidationError("second forename"))(validateName(_, "secondForename"))
